...
- **Only Two Fitted Knobs**: The entire physical downscaling operator requires just two calibrated parameters: an **advective decay scale (L = 20.8 km)** representing the downwind hydrometeor fallout distance, and an **orographic precipitation efficiency (efficiency eta = 7.7%)** capturing lateral air diversion and cloud droplet evaporation. All baseline boundaries are anchored directly to empirical observations (3,300 mm coastal baseline and 318 mm Chitradurga rain shadow floor).
...
4. **Inside the Natural Noise Floor**: Gauges that the 1D model cannot distinguish, because they share the same crest-distance to within about 1 km, disagree with each other by sigma_log approx 0.29 (+20 km cluster; pairwise ground separations 29.0–74.9 km) and 0.27 (crest cluster; ground separations 56.9–145.8 km), while the model's own scatter is **0.109** (+/- 11.5%). This representativeness noise floor applies to the 1D model specifically, demonstrating that **the model's prediction error sits comfortably inside the natural variation of indistinguishable physical gauges**.
...
- **Demonstrated Coverage Pending Gauge Calibration (8,309 Villages, Outside 12.8-15.3°N)**: The remaining 8,309 villages outside the 12.8-15.3°N validated band (3,852 northern Karnataka, 4,175 Maharashtra, 282 Goa; flagged `inside_validated_band = False`) receive continuous 250 m coverage from the identical physics operator. Maharashtra represents the largest uncalibrated exposure, with zero state gauges in the calibration set.
...
> "8,634 villages across the central Karnataka Ghats receive seasonal totals from our model, validated against nineteen gauges in that band.  
